            ...
            isEdit
              ? <>
                <label className='flex items-center' htmlFor='resumeUpload'>
                  ...
                  <input id='resumeUpload' onChange={e => setResume(e.target.files[0])} type="file" accept='application/pdf' />
        ...
        <table className='min-w-full bg-white border rounded-lg'>
          ...
          <tbody>
            {jobsApplied.map((job,index)=> true ? (
              <tr>
                <td className='py-3 px-4 flex items-centergap-2 border-b'><img className='w-8 h-8' src={job.logo} /> {job.company}</td>
