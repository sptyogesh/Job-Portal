...
const RecruiterLogin = () => {
    ...
    const[isTextDataSumbitted,SetIsTextDataSumbitted]=useState(false)

    const{setShowRecruiterLogin}=useContext(AppContext)

    const onSumbitHandler =async (e)=>{
      e.preventDefault()
      if(state=="Sign Up" && !isTextDataSumbitted){
  ...
  return (
    <div className='absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      <form onSubmit={onSumbitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
        ...
        {state==="Sign Up" && isTextDataSumbitted 
        ? <>
        ...
        <button type='submit' className='bg-blue-600 w-full text-white py-2 rounded-full mt-4'>
          {state ==='Login'?'Login': isTextDataSumbitted? 'Create Account':'Next'}
        </button>
        {state=='Login'
